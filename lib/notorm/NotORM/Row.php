...
class NotORM_Row extends NotORM_Abstract implements IteratorAggregate, ArrayAccess, Countable, JsonSerializable {
	...
	*/
	function update($data = null,$whereClause=null,$wherevalues=null) {//$whereClause=null,$wherevalues=null added by MHS
		// update is an SQL keyword
		if (!isset($data)) {
			$data = $this->modified;
		}
		$result = new NotORM_Result($this->result->table, $this->result->notORM);
		if($whereClause==null){//default
			$return = $result->where($this->result->primary, $this->primary)->update($data);
			$this->primary = $this[$this->result->primary];
		}else{//option added by MHS
			$return = $result->where($whereClause,$wherevalues)->update($data);
		}
	 ...
	 * @return int number of affected rows or false in case of an error
	 */
	function updateWhere($data = null,$where=null) {
		// update is an SQL keyword
		if (!isset($data)) {
			$data = $this->modified;
		}
